...
define(["backbone", "factory", "generator"], function(Backbone) {
    ...
    App.Views.MainView.MainConfirmView = App.Views.FactoryView.extend({
        name: 'main',
        mod: 'confirm',
        render: function() {
            ...
            this.initial_model = this.model;
            this.model = $.extend({}, this.model.toJSON(), {
                logo: App.Data.header.get('logo'),
                business_name: App.Data.header.get('business_name'),
                isOnlyGift: dining_option === 'DINING_OPTION_ONLINE'
            });

            if (get.status === "OK") {
                App.Data.myorder.order_id = get.orderId;
                var type = 'Pickup Time';
                if (dining_option === 'DINING_OPTION_DELIVERY' || dining_option === 'DINING_OPTION_DELIVERY_SEAT') {
                    type = 'Delivery Time';
                }
                this.model = $.extend(this.model, {
                    order_id: get.orderId, // order id
                    total: round_monetary_currency(get.total), // order total
                    type: type, // order type
                    time: App.Data.myorder.checkout.get('pickupTime'), // order time
                    email: App.Data.customer.get('email'),
                    status: 'success',
                    success: true,
                    reward_points: get.reward_points || '',
                    symbol: App.Data.settings.get('settings_system').currency_symbol
                });
            } else {
