...
define(["checkout_view"], function(checkout_view) {
    ...
    var CheckoutPageView = App.Views.FactoryView.extend({
        ...
        bindings: {
            ...
            '.choose-cc-box': 'updateContent: chooseCreditCardView, classes: {hide: any(not(equal(paymentMethods_selected, "credit_card_button")), not(length($tokens)))}',
            '.cc-box': 'updateContent: creditCardView, classes: {hide: any(not(equal(paymentMethods_selected, "credit_card_button")), select(length($tokens), token_selected, false))}',
