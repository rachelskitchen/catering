define(['functions'], function() {
    ...
    describe('TimeFrm', function() {
        ...
        it("Load_from_str() - 24 hour format", function() {
            var t = new TimeFrm(0, 0, '24 hour');
        ...
        it("ToString('24 hour')", function() {
            var t = new TimeFrm();

            t.load_from_str("12:00");
            expect(t.toString('24 hour')).toBe("12:00");

            t.load_from_str("12:11");
            expect(t.toString('24 hour')).toBe("12:11");

            t.load_from_str("00:00");
            expect(t.toString('24 hour')).toBe("0:00");

            t.load_from_str("00:25");
            expect(t.toString('24 hour')).toBe("0:25");

            t.load_from_str("23:59");
            expect(t.toString('24 hour')).toBe("23:59");

            t.load_from_str("04:02");
            expect(t.toString('24 hour')).toBe("4:02");

            t.load_from_str("16:13");
            expect(t.toString('24 hour')).toBe("16:13");

            t.set_minutes(60 * 24 * 3 + 60 * 2 + 7);
            expect(t.toString('24 hour')).toBe("2:07");
        });


        it("ToString('12 hour')", function() {
            var t = new TimeFrm();

            t.load_from_str("12:00");
            expect(t.toString('12 hour')).toBe("12:00pm", 'test 12:00' );

            t.load_from_str("12:11");
            expect(t.toString('12 hour')).toBe("12:11pm", 'test 12:11pm' );

            t.load_from_str("00:00");
            expect(t.toString('12 hour')).toBe("12:00am", 'test 12:00am' );

            t.load_from_str("00:25");
            expect(t.toString('12 hour')).toBe("12:25am", 'test 12:25am' );

            t.load_from_str("23:59");
            expect(t.toString('12 hour')).toBe("11:59pm", 'test 11:59am' );

            t.load_from_str("04:12");
            expect(t.toString('12 hour')).toBe("4:12am", 'test 4:12am' );

            t.load_from_str("16:13");
            expect(t.toString('12 hour')).toBe("4:13pm", 'test 4:13pm' );

            t.set_minutes(60 * 24 * 3 + 60 * 2 + 7);
            expect(t.toString('12 hour')).toBe("2:07am", 'test 3 days + 127 min' );
        });
    });
});
